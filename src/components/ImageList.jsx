...
const ImageList = () => {
   ...
   const fetchImages = async () => {
    ...
    const updateCaption = async (id) => {
      const response = await axios.put(`http://localhost:5000/api/images/${id}`, { caption: newCaption });  
    ...
    <div className='grid grid-cols-1 ms:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4'>
     {images.map(image =>(
        <div key={image._id} className='bg-slate-700 p-4 rounded-lg'>
             <img className='mb-2 h-[85%] w-auto' src={`http://localhost:5000/uploads/${image.filename}`} alt={image.caption} />
             {editing === image._id ? (
                <div>
